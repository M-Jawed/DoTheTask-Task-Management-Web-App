...
export default function Header({ boardSlug }: { boardSlug: string }) {
  const { boards, activeBoard, setActiveBoard, editModal, setEditModal } =
    useBoard();
  ...
  useEffect(() => {
    if (boards.length > 0) {
      const current = boards.find(
        (item: BoardProps) =>
          item.slug.toLowerCase() === boardSlug.toLowerCase(),
      );
      if (!current) {
  ...
  return (
    <header
      className={`z-2 w-full h-[90px] flex items-center sticky px-2 ${darkMode ? "bg-[#1A1A1D] text-white" : ""}`}
    >
      <div className="w-full flex items-center justify-between">
        ...
        <div className="w-[25%] flex items-center gap-2 px-2">
          <button
            ...
            className="w-[65%] bg-[#4682B4] px-2 py-2 text-white rounded-lg text-lg font-medium cursor-pointer hover:bg-[#0077B6]"
          >
            Add new task
          </button>
          <button
            onClick={showDeleteModal}
            className="w-[15%] py-2 px-[10px] bg-red-500 flex items-center justify-center text-xl rounded-lg cursor-pointer"
          >
            {" "}
            <MdDelete fill="white" />{" "}
          </button>
          <button
            ...
            className="w-[15%] py-2 px-[10px] bg-green-300 flex items-center justify-center text-xl rounded-lg cursor-pointer"
          >
            {" "}
            <FaEdit fill="white" />{" "}
          </button>
      ...
      {toggleEditModal && activeBoard && (
        <EditBoard
          currentBoard={activeBoard}
          editModal={toggleEditModal}
          currentBoardColumns={columns}
          closeEditModal={closeEditModal}
          onColumnsChange={refreshColumns}
        />
      )}
      {openNewTaskModal && (
        <NewTaskModal
          newTaskModal={openNewTaskModal}
